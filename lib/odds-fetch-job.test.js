...

{
  const supabase = createSupabase({
    cache: {
      baseball_mlb: { sport: 'baseball_mlb', data: [{ id: 'cached' }], fetched_at: '2026-09-08T18:00:00.000Z' },
    },
  });
  let fetched = 0;
  const out = await loadSelectedSportsOdds({
    ...
    applyBookAdjustments: identity,
    supabaseClient: supabase,
    fetchImpl: async () => { fetched += 1; return jsonRes([]); },
  });
  assert.equal(fetched, 0);
  assert.equal(out.featured[0].data[0].id, 'cached');
  assert.deepEqual(out.errors, []);
}

{
  const supabase = createSupabase({ cache: {} });
  const out = await loadSelectedSportsOdds({
    sports: ['baseball_mlb'],
    apiKey: 'k',
    applyBookAdjustments: identity,
    supabaseClient: supabase,
    fetchImpl: async () => jsonRes([{ id: 'live', bookmakers: [] }]),
  });
  assert.equal(out.featured[0].data[0].id, 'live');
  assert.equal(supabase.upserts[0].table, 'odds_cache');
}

{
  ...
  const out = await loadSelectedSportsOdds({
    ...
    applyBookAdjustments: identity,
    supabaseClient: supabase,
    cacheReadTimeoutMs: 40,
    fetchImpl: async () => jsonRes([{ id: 'live' }]),
  });
  // cache miss (empty) should hit Odds API, not hang
  assert.ok(Date.now() - t0 < 500);
  assert.equal(out.featured[0].data[0].id, 'live');
}
