...
const fliffLegs = playerTdLegsForBook([{
  sport: 'americanfootball_nfl',
  away: 'Kansas City Chiefs',
  home: 'Miami Dolphins',
  commence_time: '2026-09-27T17:00:00Z',
  ...
  players: [{
    ...
    markets: {
      anytime: {
        offers: [{ book: 'fliff', price: 999 }, { book: 'draftkings', price: 150 }],
        opp: { price: -110, book: 'fliff', count: 1, source: 'devig' },
      },
    },
  }],
}], 'draftkings', {
...
assert.equal(fliffLegs.length, 0);
const courtsideLegs = playerTdLegsForBook([{
  sport: 'americanfootball_nfl',
  away: 'Kansas City Chiefs',
  home: 'Miami Dolphins',
  commence_time: '2026-09-27T17:00:00Z',
  ...
  players: [{
    ...
    markets: {
      anytime: {
        offers: [{ book: 'courtside', price: 160 }, { book: 'draftkings', price: 150 }],
        opp: { price: -110, book: 'courtside', count: 1, source: 'devig' },
      },
    },
  }],
}], 'draftkings', {
...
assert.equal(courtsideLegs.length, 0);
const { ALL_BOOKS: promoBooks, TRUSTED_BOOK_KEYS: promoTrusted } = require('./promo-ev');
assert.equal(promoBooks.some((b) => b.key === 'fliff'), false);
assert.equal(promoTrusted.has('fliff'), false);
assert.equal(promoBooks.some((b) => b.key === 'courtside'), false);
assert.equal(promoTrusted.has('courtside'), false);
assert.equal(promoBooks.find((b) => b.key === 'betparx')?.label, 'betPARX');
assert.equal(promoBooks.find((b) => b.key === 'ballybet')?.label, 'Bally Bet');
assert.equal(promoTrusted.has('betparx'), true);
assert.equal(promoTrusted.has('ballybet'), true);
assert.equal(promoTrusted.has('hardrockbet_fl'), false);
...
const chiefs = assemblePropGame({
  ...
  bookmakers: [{
    ...
    markets: [{
      ...
      outcomes: [{ name: 'Yes', description: 'Travis Kelce', price: -500 }],
    }],
  }, {
    key: 'polymarket',
    markets: [{
      key: 'player_anytime_td',
      outcomes: [{ name: 'Yes', description: 'Travis Kelce', price: -400 }],
    }],
  }, {
    key: 'underdog',
    markets: [{
      key: 'player_anytime_td',
      outcomes: [{ name: 'Yes', description: 'Travis Kelce', price: -300 }],
    }],
  }, {
    key: 'fliff',
    markets: [{
      key: 'player_anytime_td',
      outcomes: [{ name: 'Yes', description: 'Travis Kelce', price: 999 }],
    }, {
      key: 'player_rush_reception_tds',
      outcomes: [
        { name: 'Over', description: 'Fliff Only', price: -110, point: 0.5 },
        { name: 'Under', description: 'Fliff Only', price: -110, point: 0.5 },
      ],
    }],
  }, {
    key: 'courtside',
    markets: [{
      key: 'player_anytime_td',
      outcomes: [{ name: 'Yes', description: 'Travis Kelce', price: 160 }],
    }],
  }, {
    key: 'betparx',
    markets: [{
      key: 'player_anytime_td',
      outcomes: [{ name: 'Yes', description: 'Travis Kelce', price: 145 }],
    }],
  }, {
    key: 'ballybet',
    markets: [{
      key: 'player_anytime_td',
      outcomes: [{ name: 'Yes', description: 'Travis Kelce', price: 148 }],
    }],
  }, {
    key: 'hardrockbet',
    markets: [{
      key: 'player_anytime_td',
      outcomes: [{ name: 'Yes', description: 'Travis Kelce', price: 140 }],
    }],
  }, {
    key: 'hardrockbet_fl',
    markets: [{
      ...
      outcomes: [
        ...
        { name: 'Yes', description: 'De\'Von Achane', price: 180 },
      ],
    }],
  }, {
    key: 'draftkings',
    markets: [{
      ...
      outcomes: [
        { name: 'Over', description: 'Travis Kelce', price: 150 },
        { name: 'Over', description: 'Fliff Only', price: 140 },
      ],
    }, {
      key: 'player_rush_reception_tds',
      outcomes: [
  ...
  {
    book: 'polymarket', market: 'anytime', player: "De'Von Achane", team: 'MIA',
    gameKey: kcMia, yesAmerican: -105, noAmerican: -115,
  },
], Date.parse('2026-09-26T00:00:00Z'));
const kelceProp = chiefs.players.find((p) => p.name === 'Travis Kelce');
const kelceBooks = kelceProp.markets.anytime.offers.map((row) => row.book).sort();
assert.deepEqual(kelceBooks, ['ballybet', 'betparx', 'draftkings', 'hardrockbet', 'kalshi']);
assert.equal(kelceProp.markets.anytime.offers.filter((row) => row.book === 'hardrockbet').length, 1);
assert.equal(kelceProp.markets.anytime.offers.find((row) => row.book === 'hardrockbet').price, 140);
assert.equal(kelceProp.markets.anytime.offers.some((row) => row.book === 'hardrockbet_fl'), false);
assert.equal(kelceProp.markets.anytime.offers.some((row) => row.book === 'courtside' || row.price === 160), false);
assert.equal(kelceProp.markets.anytime.offers.find((row) => row.book === 'draftkings').price, 150);
assert.equal(kelceProp.markets.anytime.offers.find((row) => row.book === 'kalshi').price, -170);
assert.equal(kelceProp.markets.anytime.offers.some((row) => row.price === -500 || row.price === -400 || row.price === -300 || row.book === 'fliff' || row.book === 'courtside'), false);
assert.equal(chiefs.players.some((p) => p.name === 'Fliff Only'), false);
...
assert.equal(chiefs.players.some((p) => p.name === 'Tyreek Hill'), false);
assert.equal(chiefs.players.some((p) => p.name === 'Yes'), false);
const achane = chiefs.players.find((p) => p.name === "De'Von Achane");
const achaneBooks = achane.markets.anytime.offers.map((row) => row.book).sort();
assert.deepEqual(achaneBooks, ['hardrockbet', 'polymarket']);
assert.equal(achane.markets.anytime.offers.find((row) => row.book === 'hardrockbet').price, 180);
assert.equal(achane.markets.anytime.offers.some((row) => row.book === 'hardrockbet_fl'), false);
assert.equal(canonicalBookKey('hardrockbet_fl'), 'hardrockbet');
assert.equal(canonicalBookKey('HardRockBet_FL'), 'hardrockbet');

const url = propEventUrl('abc', 'secret-key');
assert.match(url, /regions=us,us2(&|$)/);
assert.equal(PROP_REGIONS, 'us,us2');
assert.equal(PROP_CREDITS_PER_EVENT, 2);
assert.equal((url.match(/regions=/g) || []).length, 1);
assert.equal(url.includes('us_ex'), false);
assert.equal(url.includes('eu'), false);
assert.equal(url.includes('bookmakers='), false);
...
// Kalshi stamps the Eastern kickoff with a Pacific offset (+3h). 20:00Z is
// 4:00 PM ET; the game kicks at 1:00 PM ET (17:00Z). Polymarket and Underdog
// already send 17:00Z, and that time wins.
const carCle = 'CAR|CLE|2026-09-27';
const kickoffBoard = boardFromQuotes([
  {
    book: 'kalshi', market: 'anytime', player: 'Chuba Hubbard', team: 'CAR',
    away: 'CAR', home: 'CLE', gameKey: carCle,
    commence: '2026-09-27T20:00:00Z', yesAmerican: -150,
  },
  {
    book: 'polymarket', market: 'anytime', player: 'Chuba Hubbard', team: 'CAR',
    away: 'CAR', home: 'CLE', gameKey: carCle,
    commence: '2026-09-27T17:00:00Z', yesAmerican: -140,
  },
  {
    book: 'underdog_predict', market: 'anytime', player: 'Chuba Hubbard', team: 'CAR',
    away: 'CAR', home: 'CLE', gameKey: carCle,
    commence: '2026-09-27T17:00:00Z', yesAmerican: -130,
  },
]);
assert.equal(kickoffBoard[0].commence, '2026-09-27T17:00:00.000Z');
assert.equal(formatKickoffEt(kickoffBoard[0].commence), '1:00 PM ET');
const larOnly = boardFromQuotes([
  {
    book: 'kalshi', market: 'anytime', player: 'Puka Nacua', team: 'LAR',
    away: 'LAR', home: 'DEN', gameKey: 'LAR|DEN|2026-09-27',
    commence: '2026-09-28T03:20:00Z', yesAmerican: -160,
  },
]);
assert.equal(larOnly[0].commence, '2026-09-28T00:20:00.000Z');
assert.equal(formatKickoffEt(larOnly[0].commence), '8:20 PM ET');
const trueSoon = new Date(now + 70 * 3600 * 1000).toISOString();
const kalshiLate = new Date(now + 73 * 3600 * 1000).toISOString();
const rewound = selectPlayerTdQuotes([
  { book: 'kalshi', market: 'anytime', player: 'Puka Nacua', gameKey: 'LAR|DEN|2026-09-27', commence: kalshiLate, yesAmerican: -160 },
], now);
assert.equal(rewound.length, 1);
assert.equal(canonicalKickoffMs(rewound), Date.parse(trueSoon));
const alreadyKicked = selectPlayerTdQuotes([
  {
    book: 'kalshi', market: 'anytime', player: 'Chuba Hubbard', gameKey: carCle,
    commence: new Date(now + 2 * 3600 * 1000).toISOString(), yesAmerican: -150,
  },
], now);
assert.equal(alreadyKicked.length, 0);

const udNow = Date.parse('2026-09-26T12:00:00Z');
const freshUd = new Date(udNow - 2 * 3600 * 1000).toISOString();
const oldUd = new Date(udNow - 25 * 3600 * 1000).toISOString();
const udGame = assemblePropGame({
  id: 'carcle',
  ...
  bookmakers: [{
    ...
    markets: [{
      key: 'player_anytime_td',
      outcomes: [{ name: 'Yes', description: 'Chuba Hubbard', price: 140 }],
    }],
  }],
}, [
  {
    book: 'underdog_predict', market: 'anytime', player: 'Chuba Hubbard', team: 'CAR',
    gameKey: gameKeyFromTeams('Carolina Panthers', 'Cleveland Browns', '2026-09-27T17:00:00Z'),
    yesAmerican: -120, noAmerican: 100, updatedAt: freshUd,
  },
  {
    book: 'underdog_predict', market: 'anytime', player: 'Old Player', team: 'CAR',
    gameKey: gameKeyFromTeams('Carolina Panthers', 'Cleveland Browns', '2026-09-27T17:00:00Z'),
    yesAmerican: 250, noAmerican: -300, updatedAt: oldUd,
  },
  {
    book: 'kalshi', market: 'anytime', player: 'Chuba Hubbard', team: 'CAR',
    gameKey: gameKeyFromTeams('Carolina Panthers', 'Cleveland Browns', '2026-09-27T17:00:00Z'),
    yesAmerican: -110, noAmerican: -110,
  },
], udNow);
const hubbard = udGame.players.find((p) => p.name === 'Chuba Hubbard');
assert.equal(hubbard.markets.anytime.offers.some((row) => row.book === 'underdog_predict' && row.price === -120), true);
assert.equal(udGame.players.some((p) => p.name === 'Old Player'), false);

const { buildAllLegsForBook } = require('./promo-ev');
const promoUd = buildAllLegsForBook({
  playerTds: [{
    sport: 'americanfootball_nfl',
    ...
    players: [{
      ...
      markets: {
        anytime: {
          offers: [{ book: 'underdog_predict', price: -120, updatedAt: freshUd }],
          opp: { price: -110, book: 'kalshi', count: 1, source: 'exchange' },
        },
      },
    }, {
      name: 'Old Player',
      markets: {
        anytime: {
          offers: [{ book: 'underdog_predict', price: 250, updatedAt: oldUd }],
          opp: { price: -300, book: 'kalshi', count: 1, source: 'exchange' },
        },
      },
    }],
  }],
}, 'underdog_predict', null, null, 'any', null, { now: udNow });
assert.equal(promoUd.some((leg) => leg.name === 'Chuba Hubbard 1+ TD'), true);
assert.equal(promoUd.some((leg) => leg.name === 'Old Player 1+ TD'), false);
