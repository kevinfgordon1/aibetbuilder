...
assert.deepEqual(kelceBooks, ['ballybet', 'betparx', 'courtside', 'draftkings', 'hardrockbet', 'kalshi']);
...
assert.equal(kelceProp.markets.anytime.offers.find((row) => row.book === 'courtside').price, 160);
...
assert.equal(kelceProp.markets.anytime.offers.some((row) => row.price === -500 || row.price === -400 || row.price === -300 || row.book === 'fliff'), false);
