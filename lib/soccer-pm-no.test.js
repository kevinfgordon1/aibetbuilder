...
(async () => {
  ...
  const atlCatalog = [{
    ...
    markets: [
      {
        ticker: 'KXMLSGAME-DC',
        yes_sub_title: 'DC United',
        no_ask_dollars: '0.4000',
        yes_bid_size_fp: '111.00',
      },
      {
        ticker: 'KXMLSGAME-ATL',
        yes_sub_title: 'Atlanta',
        no_ask_dollars: '0.6100',
        yes_bid_size_fp: '222.00',
      },
      {
        ticker: 'KXMLSGAME-TIE',
        yes_sub_title: 'Tie',
        no_ask_dollars: '0.7200',
        yes_bid_size_fp: '333.00',
      },
    ],
  }];
  const atlHit = matchKalshiMarket({
    market: 'ML', team: 'Atlanta United FC', buySide: 'no',
    away: 'Atlanta United FC', home: 'D.C. United',
  }, atlCatalog);
  assert.equal(atlHit.ticker, 'KXMLSGAME-ATL', 'Odds API Atlanta United FC → Kalshi Atlanta No');
  assert.notEqual(atlHit.ticker, 'KXMLSGAME-DC');
  const atlGames = [{
  ...
  const atlQuotes = await resolveKalshiSoccerNos(atlGames, {
    eventsBySport: { soccer_usa_mls: atlCatalog },
  });
  const atlRec = atlQuotes[soccerPmGameKey(atlGames[0])];
  assert.ok(atlRec.home && atlRec.away && atlRec.draw);
  const atlNo = kalshiCatalogTop(atlCatalog[0].markets[1], 'no');
  const dcNo = kalshiCatalogTop(atlCatalog[0].markets[0], 'no');
  assert.equal(atlRec.away.best, atlNo.american, 'Atlanta United Yes hedges Atlanta No, not DC United No');
  assert.equal(atlRec.home.best, dcNo.american);
  assert.notEqual(atlRec.away.best, dcNo.american);
  const atlRaw = [{
    commence_time: future,
    ...
    bookmakers: [{
      ...
      markets: [{
        key: 'h2h',
        outcomes: [
          { name: 'Atlanta United FC', price: 150 },
          { name: 'Draw', price: 240 },
          { name: 'DC United', price: 170 },
        ],
      }],
    }],
  }];
  const atlOver = overlaySoccerPmNos(transformOddsData(atlRaw, 'soccer_usa_mls'), atlQuotes);
  assert.equal(atlOver.moneylines[0].best_away_no, atlNo.american);
  assert.equal(atlOver.moneylines[0].best_away_no_book, 'kalshi');
  const atlLeg = buildAllLegsForBook(atlOver, 'draftkings').find((l) => l.name === 'Atlanta United FC ML');
  assert.equal(atlLeg.bestOppName, 'Atlanta United FC ML No');
  assert.equal(atlLeg.bestOpp, atlNo.american);
  assert.notEqual(atlLeg.bestOpp, dcNo.american);
