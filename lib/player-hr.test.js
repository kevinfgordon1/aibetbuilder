...
}
{
  ...
  // DraftKings / FanDuel / BetMGM / Fanatics post 1+ HR as a milestone line,
  // which The Odds API files under batter_home_runs_alternate (Over 0.5).
  // The alternate price fills in when that book has no main-market price;
  // a book's batter_home_runs price wins for the same player.
  const ALT = {
    ...LAA_SEA,
    bookmakers: [
      { key: 'draftkings', markets: [{ key: 'batter_home_runs_alternate', last_update: '2026-09-26T14:58:00Z', outcomes: [
        { name: 'Over', description: 'Julio Rodriguez', price: 390, point: 0.5 },
        { name: 'Over', description: 'Julio Rodriguez', price: 2100, point: 1.5 },
        { name: 'Over', description: 'Cal Raleigh', price: 260, point: 0.5 },
      ] }] },
      { key: 'betmgm', markets: [
        { key: 'batter_home_runs', outcomes: [{ name: 'Over', description: 'Julio Rodriguez', price: 370, point: 0.5 }] },
        { key: 'batter_home_runs_alternate', outcomes: [
          { name: 'Over', description: 'Julio Rodríguez', price: 450, point: 0.5 },
          { name: 'Over', description: 'Cal Raleigh', price: 240, point: 0.5 },
        ] },
      ] },
      { key: 'fliff', markets: [{ key: 'batter_home_runs_alternate', outcomes: [{ name: 'Over', description: 'Julio Rodriguez', price: 900, point: 0.5 }] }] },
    ],
  };
  const altOffers = job.sportsbookHrOffers(ALT).map((o) => [o.book, o.player, o.price]);
  assert.deepEqual(altOffers, [
    ['draftkings', 'Julio Rodriguez', 390],
    ['draftkings', 'Cal Raleigh', 260],
    ['betmgm', 'Julio Rodriguez', 370],
    ['betmgm', 'Cal Raleigh', 240],
  ], 'alternate Over 0.5 only; main market wins per book; Fliff never an offer');
  const altGame = job.assembleHrGame(ALT, quotes, NOW);
  const altJulio = altGame.players.find((p) => p.name === 'Julio Rodriguez');
  assert.deepEqual(altJulio.markets.hr.offers.map((o) => [o.book, o.price]).sort(), [['betmgm', 370], ['draftkings', 390]]);
  const altLegs = td.playerTdLegsForBook(td.playerTdsFromCacheRows([{ event_id: altGame.eventId, sport: 'baseball_mlb', commence_time: altGame.commence_time, away_team: altGame.away, home_team: altGame.home, data: altGame }]), 'draftkings', { now: NOW });
  assert.deepEqual(altLegs.map((l) => [l.name, l.dk, l.market]).sort(), [['Cal Raleigh 1+ HR', 260, 'HR'], ['Julio Rodriguez 1+ HR', 390, 'HR']]);
...
(async () => {
  ...
  // ── runPlayerHrJob: today's games only, gated on exchange quotes, credits summed
  {
    ...
    assert.match(seen[0], /sports\/baseball_mlb\/events\/ev-laa-sea\/odds\/.*regions=us,us2&markets=batter_home_runs,batter_home_runs_alternate&/);
    ...
    assert.deepEqual(row.markets, ['batter_home_runs', 'batter_home_runs_alternate']);
