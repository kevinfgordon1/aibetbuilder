...
// includes Fliff. Player TD fair prices and picks drop the Odds API key too,
// including rows already stored before this filter.
const PLAYER_TD_EXCLUDED_BOOKS = Object.freeze(['fliff']);
...
function selectPlayerTdQuotes(quotes, now, markets = ENABLED_TD_MARKETS) {
  ...
  for (const list of byGame.values()) {
    ...
    for (const quote of list) {
      const t = Date.parse(quote.commence || '');
      if (!Number.isFinite(t)) continue;
      if (kick == null || t < kick) kick = t;
    }
...
function boardFromQuotes(quotes) {
  ...
  for (const quote of quotes || []) {
    ...
    if (!game) {
      ...
      if (!game.commence && quote.commence) game.commence = quote.commence;
    }
