...
//
// Sportsbook side: Odds API batter_home_runs, Over 0.5 only (= 1+ HR), for
// MLB games that start later today (ET). Regions us,us2 in one request
// (1 market x 2 regions = 2 credits per event). Only events where an
...
const HR_MARKETS = Object.freeze([HR_MARKET_KEY]);
const HR_REGIONS = 'us,us2';
const HR_CREDITS_PER_EVENT = 2;
...
function hrEventUrl(eventId, apiKey) {
  ...
  return `https://api.the-odds-api.com/v4/sports/${HR_SPORT}/events/${id}/odds/?apiKey=${apiKey}&regions=${HR_REGIONS}&markets=${HR_MARKET_KEY}&oddsFormat=american&includeBetLimits=true`;
}
...
// Odds API batter_home_runs: outcomes are Over / Under with the player in
// description and point 0.5 for 1+. Other points (1.5 = 2+) are ignored.
function sportsbookHrOffers(event) {
  ...
  for (const book of (event && event.bookmakers) || []) {
    ...
    for (const market of book.markets || []) {
      if (!market || market.key !== HR_MARKET_KEY) continue;
      ...
      for (const outcome of market.outcomes || []) {
        ...
        if (!name) continue;
        out.push({ book: bookKey, player: name, price: Number(outcome.price), updatedAt });
      }
    }
  }
  return out;
