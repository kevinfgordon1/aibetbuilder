...
function matchKalshiMarket(sel, events) {
  ...
  if (sel.market === 'ML') {
    ...
    const wantDraw = /^draw$/i.test(sel.team);
    for (const m of markets) {
      const lab = m.yes_sub_title || m.subtitle || m.title || '';
      if (wantDraw && /\b(draw|tie)\b/i.test(lab)) return { ticker: m.ticker, buySide, market: m };
      if (!wantDraw && tokensHit(nameTokens(lab), want)) return { ticker: m.ticker, buySide, market: m };
    }
    return null;
  }
  if (sel.market === 'SPR') {
...
function polyMarketMatches(sel, market) {
  ...
  if (sel.market === 'ML') {
    ...
    const teamHit = tokensHit(nameTokens(q), nameTokens(sel.team))
      || (/^draw$/i.test(sel.team) && /\bdraw\b|\btie\b/i.test(ql));
    if (isBinaryYesNo && teamHit) {
      return { tokenSide: sel.buySide === 'no' ? 'no' : 'yes' };
    }
