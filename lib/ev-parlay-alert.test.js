...
// ── scan is MLB + 7 Days only: drop NFL / far-future / already-started legs
{
  ...
  const mkGame = (away, home, commence_time, dkAway, dkHome, fdAway, fdHome) => ({
    commence_time,
    away_team: away,
    home_team: home,
    bookmakers: [
  ...
  const mlbWindow = [
    mkGame("Alpha", "Beta", inWindow, 150, -130, -180, 160),
    mkGame("Gamma", "Delta", inWindow, 140, -125, -175, 155),
    mkGame("Epsilon", "Zeta", inWindow, 145, -128, -170, 150),
  ];
  const mlbExcluded = [
    mkGame("FarA", "FarB", far, 150, -130, -180, 160),
    mkGame("PastA", "PastB", started, 150, -130, -180, 160),
  ];
  const nflWindow = [
    mkGame("NFL1", "NFL2", inWindow, 150, -130, -180, 160),
    mkGame("NFL3", "NFL4", inWindow, 140, -125, -175, 155),
    mkGame("NFL5", "NFL6", inWindow, 145, -128, -170, 150),
  ];
  const oddsData = hydrateFeaturedOdds([
    { sport: "baseball_mlb", data: [...mlbWindow, ...mlbExcluded] },
    { sport: "americanfootball_nfl", data: nflWindow },
    { sport: "basketball_nba", data: nflWindow.map((g, i) => ({ ...g, away_team: `NBA${i}A`, home_team: `NBA${i}B` })) },
  ]);
  const scan = scanBooksForEvParlays(oddsData, {
    books: [{ key: "draftkings", label: "DraftKings" }],
    candidateCap: 8,
    timeBudgetMs: 10000,
  });
  assert.ok(scan.comboCount > 0, "three in-window MLB games should still produce combos");
  assert.ok(scan.parlays.length >= 1);
  const sevenDays = Date.now() + 7 * 24 * 60 * 60 * 1000;
  for (const p of scan.parlays) {
    for (const l of p.legs) {
      assert.equal(l.sport, "baseball_mlb");
      assert.ok(!/NFL|NBA|Far|Past/.test(l.name));
      const ct = new Date(l.commence_time).getTime();
      assert.ok(ct > Date.now());
      assert.ok(ct <= sevenDays);
    }
  }

  const nflOnly = hydrateFeaturedOdds([{ sport: "americanfootball_nfl", data: nflWindow }]);
  const nflScan = scanBooksForEvParlays(nflOnly, {
    books: [{ key: "draftkings", label: "DraftKings" }],
    candidateCap: 8,
    timeBudgetMs: 10000,
  });
  assert.equal(nflScan.parlays.length, 0);
  assert.ok(nflScan.stats.every(s => s.skipped || s.legs === 0 || s.hits === 0));
}
...
// ── recipient fallback: Kevin's user id as @evparlaysbot chat_id only (not KayGo bot)
{
  ...
  assert.equal(SCAN_DATE_RANGE, "7d");
  assert.ok(alertSrc.includes("SCAN_SPORT_KEYS"));
  assert.ok(alertSrc.includes("SCAN_DATE_RANGE"));
  assert.ok(!alertSrc.includes('null, null, "any"'));
}
