...
const {
  ...
  SCAN_DATE_RANGE,
  EV_ALERT_BOOKS,
  EV_ALERT_EXCLUDE_BOOK_KEYS,
} = require("./ev-parlay-alert");
const { ALL_BOOKS, calcParlayEV, evPct, passesEvThreshold, hydrateFeaturedOdds, SPORT_KEYS } = require("./promo-ev");
const {
...
// ── 3-leg search skips same game and respects EV% > 3
{
  assert.equal(MIN_EV_PCT, 3);
  ...
  for (const p of parlays) {
    ...
    assert.ok(passesEvThreshold(p.ev, 100, 3));
    assert.ok(p.evPct > 3);
  }
...
// ── BetOpenly is not a default EV-alert book even if us_ex still returns it
{
  assert.ok(ALL_BOOKS.some(b => b.key === "betopenly"), "Promo Builder ALL_BOOKS keeps betopenly");
  assert.ok(EV_ALERT_EXCLUDE_BOOK_KEYS.has("betopenly"));
  assert.ok(!EV_ALERT_BOOKS.some(b => b.key === "betopenly"), "default scanner list must drop betopenly");
  const games = mlbPlusEvSlate().map(g => ({
    ...g,
    bookmakers: g.bookmakers.concat([{
      key: "betopenly",
      markets: [{ key: "h2h", outcomes: [
        { name: g.away_team, price: 200 },
        { name: g.home_team, price: 180 },
      ] }],
    }]),
  }));
  const scan = scanBooksForEvParlays(
    hydrateFeaturedOdds([{ sport: "baseball_mlb", data: games }]),
    { candidateCap: 8, timeBudgetMs: 10000 }
  );
  assert.ok(!scan.parlays.some(p => p.bookKey === "betopenly"));
  assert.ok(!scan.parlays.some(p => p.legs.some(l => l.bookKey === "betopenly")));
  assert.ok(!scan.stats.some(s => s.book === "betopenly"));
}
...
// ── new bot token env name only (never KayGo TELEGRAM_BOT_TOKEN, never EV_ALERT_TELEGRAM_BOT_TOKEN)
{
  ...
  const jobSrc = fs.readFileSync(path.join(__dirname, "./ev-parlay-scan-job.js"), "utf8");
  const workerSrc = fs.readFileSync(path.join(__dirname, "../worker/ev-parlay-alerts.js"), "utf8");
  ...
  for (const src of [scanSrc, jobSrc, workerSrc, hookSrc, tokenSrc, flowSrc]) {
    assert.ok(!src.includes("EV_ALERT_TELEGRAM_BOT_TOKEN"));
    assert.ok(!src.includes("process.env.TELEGRAM_BOT_TOKEN"));
  }
  for (const src of [scanSrc, jobSrc, workerSrc, hookSrc, tokenSrc]) {
...
// ── recipient fallback: Kevin's user id as @evparlaysbot chat_id only (not KayGo bot)
{
  ...
  assert.ok(scanSrc.includes("runEvParlayScan"));
  assert.ok(jobSrc.includes("deliverEvParlayAlerts"));
  assert.ok(jobSrc.includes("resolveEvParlaysBotToken"));
  assert.ok(jobSrc.includes("loadOddsCacheBySport"));
  assert.ok(jobSrc.includes("SCAN_SPORT_KEYS"));
  assert.ok(jobSrc.includes("ODDS_CACHE_COLUMNS"));
  assert.ok(jobSrc.includes(".eq('sport', sport)"));
  assert.ok(jobSrc.includes("maybeSingle()"));
  assert.ok(!/\bSPORT_KEYS\b/.test(jobSrc));
  assert.ok(!jobSrc.includes(".in('sport'"));
  assert.ok(!jobSrc.includes(".select('*')"));
  assert.match(alertSrc, /SCAN_SPORT_KEYS = \["baseball_mlb"\]/);
  assert.match(alertSrc, /SCAN_DATE_RANGE = "24h"/);
  assert.match(alertSrc, /MIN_EV_PCT = 3/);
  assert.ok(scanSrc.includes("EV% > 3"));
  assert.ok(hookSrc.includes("EV% > 3"));
  ...
  assert.ok(!jobSrc.includes("process.env.TELEGRAM_BOT_TOKEN"));
