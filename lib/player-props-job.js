...
// Separate Odds API pull for NFL player touchdowns. The featured cron's
// 20s budget cannot walk per-event prop calls. This job stores sportsbook
// Yes prices plus a fair opposite (best exchange No, else a two-sided
// de-vig) on player_prop_cache. It logs x-requests-* and never logs the key.

const { TRUSTED_BOOK_KEYS } = require('./promo-ev');
const { UNDERDOG_STALE_MS } = require('./underdog-freshness');
const { loadExchangeTdQuotes } = require('./player-td-feeds');
const {
  gameKeyFromTeams,
  samePlayer,
  samePlayerInGame,
...
const PROP_REGIONS = 'us,us2,us_ex,eu';
const MAX_PROP_EVENTS = 16;
const PROP_WINDOW_BEFORE_MS = 6 * 60 * 60 * 1000;
const PROP_WINDOW_AFTER_MS = 24 * 60 * 60 * 1000;
const PROP_FETCH_CONCURRENCY = 4;
const ODDS_API_TIMEOUT_MS = 8000;

const EXCHANGE_BOOKS = new Set(['kalshi', 'polymarket', 'underdog_predict', 'novig', 'prophetx']);
...
function eventInPropWindow(game, now) {
  const t = Date.parse(game && game.commence_time);
  if (!Number.isFinite(t)) return false;
  const clock = Number(now);
  return t >= clock - PROP_WINDOW_BEFORE_MS && t <= clock + PROP_WINDOW_AFTER_MS;
}
...
function marketBucket(marketKey, point) {
  ...
  if (marketKey === 'player_tds_over' || marketKey === 'player_rush_reception_tds') {
...
function assemblePropGame(event, exchangeQuotes, now) {
  ...
  for (const book of event.bookmakers || []) {
    if (!book || EXCHANGE_BOOKS.has(book.key)) continue;
    if (!TRUSTED_BOOK_KEYS.has(book.key)) continue;
    for (const market of book.markets || []) {
      ...
      for (const row of byPlayer.values()) {
        ...
        const twoSided = market.key === 'player_tds_over' || market.key === 'player_rush_reception_tds';
        if (row.yes != null) addOffer(slot, book.key, row.yes, updatedAt, dedicated);
        if (row.yes != null && row.no != null && twoSided) {
          slot.pairs.push({ book: book.key, over: row.yes, under: row.no });
        }
      }
    }
  }
  ...
  for (const quote of quotes) {
    if (!TD_MARKETS.includes(quote.market) || quote.yesAmerican == null) continue;
    const player = ensurePlayer(players, gameKey, quote.player, quote.team);
    const slot = player.markets[quote.market];
    const staleUnderdog = quote.book === 'underdog_predict' && quoteIsOlderThan(quote.updatedAt, now, UNDERDOG_STALE_MS);
  ...
  for (const player of players) {
    const markets = {};
    for (const market of TD_MARKETS) {
      ...
      for (const quote of quotes) {
        ...
        if (quote.book === 'underdog_predict' && quoteIsOlderThan(quote.updatedAt, now, UNDERDOG_STALE_MS)) continue;
        nos.push({ price: quote.noAmerican, book: quote.book, source: 'exchange' });
      }
      const bestNo = bestAmericanQuote(nos);
      const opp = bestNo
        ? { price: bestNo.price, book: bestNo.book, count: nos.length, source: 'exchange' }
        : pickDevig(slot.pairs);
...
async function runPlayerPropsJob({
  ...
  const pulled = await mapPool(picked, PROP_FETCH_CONCURRENCY, async (game) => {
    ...
    return { id: game.id, game: assemblePropGame(event, quotes, now) };
  });
  ...
  if (supabaseClient) {
    try {
      await supabaseClient
        ...
        .delete()
        .eq('sport', PROP_SPORT)
        .lt('commence_time', new Date(now - PROP_WINDOW_BEFORE_MS).toISOString());
    } catch (_) { /* best-effort */ }
  }
