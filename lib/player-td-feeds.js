...
const KALSHI_SERIES = Object.freeze(['KXNFLTD', 'KXNFLFIRSTTD']);
...
function kalshiQuoteFromMarket(market, nowMs) {
  ...
  return {
    ...
    commence: market.occurrence_datetime || market.expected_expiration_time || null,
...
async function loadExchangeTdQuotes(deps) {
  ...
  const keys = new Set();
  for (const quote of [...kalshi, ...underdog]) {
  ...
  return [...kalshi, ...polymarket, ...underdog].filter((quote) => quote && quote.yesAmerican != null);
}
