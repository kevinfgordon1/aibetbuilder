...
const POLY_BOOK_CONCURRENCY = 3;
const POLY_BOOK_GAP_MS = 125;       // ~8 req/s, under the 20/s public limit
const POLY_BOOK_BUDGET_MS = 20000;
const MAX_POLY_BOOKS = 160;
...
// Polymarket US public limit is 20 req/s per IP, shared with the board and
// stream routes on the same Vercel egress. Pace well under it, honor
// Retry-After on 429, and stop starting new requests at the time budget.
async function loadPolymarketNoLadders(slugs, deps) {
  const fetchFn = (deps && deps.fetchFn) || fetch;
  const base = (deps && deps.polymarketBase) || POLY_GATEWAY;
  const sleepFn = (deps && deps.sleepFn) || sleep;
  const nowFn = (deps && deps.nowFn) || Date.now;
  const cap = deps && deps.maxPolyBooks != null ? deps.maxPolyBooks : MAX_POLY_BOOKS;
  const gapMs = deps && deps.polyGapMs != null ? deps.polyGapMs : POLY_BOOK_GAP_MS;
  const budgetMs = deps && deps.polyBudgetMs != null ? deps.polyBudgetMs : POLY_BOOK_BUDGET_MS;
  const list = [...new Set((slugs || []).filter(Boolean))].slice(0, cap);
  const out = new Map();
  const started = nowFn();
  ...
  async function worker() {
    while (cursor < list.length) {
      if (nowFn() - started > budgetMs) return;
      const slug = list[cursor];
      cursor += 1;
      const wait = nextStart - nowFn();
      nextStart = Math.max(nowFn(), nextStart) + gapMs;
      ...
      if (status === 429) {
        ...
        nextStart = Math.max(nextStart, nowFn() + pause);
        if (throttled >= 3) return; // do not keep hammering a throttled IP
        continue;
      }
...
async function attachNoLadders(quotes, deps) {
  ...
  return { kalshi: kalshi.length, polymarket: poly.length, kalshiBooks: kBooks.size, polymarketBooks: pBooks.size, attached };
}
