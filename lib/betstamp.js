...
const TRIAL_BOOK_IDS = [100, 200, 300, 250, 613, 642, 150, 365, 191, 193, 194, 196];
...
function snapshotCacheControl(remainingMs) {
...
async function fetchSnapshotWithCache(query, deps = {}) {
  ...
  const ttlMs = deps.cacheTtlMs != null ? deps.cacheTtlMs : SNAPSHOT_CACHE_TTL_MS;
  ...
  const work = (async () => {
    ...
    writeSnapshotCacheEntry(cache, key, snap, storedAt, ttlMs);
    return {
      snap,
      cacheStatus: 'MISS',
      ageMs: 0,
