...
// odds_cache is required, loaded one sport at a time, and hard-capped (~25s
// per sport). A sport that times out is skipped; zero sports → 504.
const {
  STAKE,
  selectNewAlerts,
  formatAlertMessage,
...
// Vercel leftover HTTP endpoint only. MLB's `data` JSON over PostgREST from
// Vercel often exceeds 8s; 25s is still well under maxDuration: 60 so a stuck
// PostgREST/statement cannot hold that invocation open with zero bytes.
// The Railway worker does NOT use this cap — it injects ~120s via
// EV_ODDS_CACHE_TIMEOUT_MS / WORKER_ODDS_CACHE_TIMEOUT_MS.
const ODDS_CACHE_TIMEOUT_MS = 25000;
...
function normalizeOddsCacheResult(result) {
  if (result && result.error) return result;
  const row = result && result.data != null ? result.data : null;
  const data = row == null ? [] : Array.isArray(row) ? row : [row];
  return { data, error: null };
}

/**
 * Load odds_cache one sport at a time so a combined JSON payload cannot 504
 * the whole scan. Each sport has its own abort timeout (~25s). Timed-out
 * sports are skipped; if none load, the caller should 504.
 */
async function loadOddsCacheBySport(sports, querySport, { timeoutMs = ODDS_CACHE_TIMEOUT_MS } = {}) {
  const rows = [];
  const loadedSports = [];
  const timedOutSports = [];
  const errorSports = [];

  for (const sport of sports || []) {
    const out = await runRequired(
      async (signal) => normalizeOddsCacheResult(await querySport(sport, signal)),
      { timeoutMs, label: `odds_cache ${sport}` }
    );
    if (out.timedOut) {
      timedOutSports.push(sport);
      continue;
    }
    if (!out.ok) {
      errorSports.push(sport);
      continue;
    }
    loadedSports.push(sport);
    if (out.data && out.data.length) rows.push(...out.data);
  }

  console.log('scan-ev-parlays odds_cache:', {
    loaded: loadedSports,
    timedOut: timedOutSports,
    errors: errorSports,
    rows: rows.length,
  });

  if (!loadedSports.length) {
    const timedOut = timedOutSports.length > 0;
    return {
      ok: false,
      status: 504,
      data: [],
      ...
      errorSports,
      message: timedOut
        ? `odds_cache timed out after ${timeoutMs}ms (${timedOutSports.join(', ')})`
        : 'odds_cache failed for all sports',
    };
  }
