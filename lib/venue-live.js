...
function startKalshiPoll(league, deps, emit) {
  ...
  const loop = (async () => {
    ...
    while (!stopped) {
      ...
      // Ship the whole book, not the tickers that moved. A client that
      // connects to a warm poll (or misses the first event) otherwise
      // paints only the last contract whose ask changed.
      if (polls === 0 || changed.length) emit(quotes);
      polls += 1;
...
function ssePacket(value, fallbackMode) {
  ...
  return { quotes: Array.isArray(value) ? value : [], mode: fallbackMode, note: undefined };
}
...
function attachHub(res, req, { hubs, key, source, mode, start }) {
  ...
  const write = (incoming) => {
    ...
    try {
      res.write(formatQuoteSse(packet.quotes, Date.now(), { source, mode: packet.mode, note: packet.note }));
