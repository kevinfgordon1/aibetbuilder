...
function startKalshiPoll(league, deps, emit) {
  ...
  const loop = (async () => {
    ...
    while (!stopped) {
      ...
      if (stopped) break;
      // Every poll ships the whole moneyline book, even when no ask moved.
      // Emitting only the ticker that changed left a connected board stuck
      // on the pregame print for every other game (and for this one, once
      // that connection missed the delta).
      if (quotes.length) emit(quotes);
      polls += 1;
      if (maxPolls != null && polls >= maxPolls) break;
      await sleep(pollMs, () => stopped);
    }
