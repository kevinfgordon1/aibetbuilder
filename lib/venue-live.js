...
// The price to buy an outcome is its best ask.
// CLOB POST /prices { side: "BUY" } returns the best bid. { side: "SELL" }
// returns the best ask (confirmed against /book: BUY == max bid, SELL == min ask).
// The websocket best_ask field is that same ask. Gamma outcomePrices are a
// cached mid and are only a fallback when CLOB returns nothing.
async function loadPolymarketAsks(entries, deps) {
  ...
  const res = await fetchJson(fetchFn, `${POLY_CLOB}/prices`, {
    method: 'POST',
    body: JSON.stringify(list.map((entry) => ({ token_id: entry.tokenId, side: 'SELL' }))),
  });
  ...
  for (const entry of list) {
    ...
    const ask = row && typeof row === 'object' ? (row.SELL != null ? row.SELL : row.sell) : row;
...
function startPolymarket(league, deps, emit) {
  ...
  const run = (async () => {
    ...
    while (!stopped) {
      ...
      let priced = [];
      try {
        priced = await loadPolymarketAsks(entries, Object.assign({}, deps, { now: fetchedAt }));
      } catch (_) {
        priced = [];
      }
...
async function loadKalshiQuotes(league, deps) {
  ...
  return quotesFromKalshiEvents(events, league, deps && deps.now);
}
