...
// orderbook_snapshot replaces the no-bid ladder. orderbook_delta adds the
// signed size and deletes the level at 0. Yes ask = 1 − best no bid.
function quotesFromKalshiOrderbook(message, books, metaByTicker) {
  ...
  if (type === 'orderbook_snapshot') {
    no = new Map();
    const rows = msg.no_dollars || msg.no || [];
  ...
  const ladder = [...no.entries()].map(([price, size]) => [price, size]);
  const ask = kalshiYesAskFromNoBids(ladder);
  const odds = asProb(ask);
  if (odds == null) return [];
  return [{
    ...meta,
...
function startKalshiPoll(league, deps, emit) {
  ...
  const openWs = () => {
    ...
    socket.addEventListener('message', (ev) => {
      ...
      let parsed;
      try { parsed = JSON.parse(text); } catch (_) { return; }
      const updates = quotesFromKalshiOrderbook(parsed, noBooks, book).concat(quotesFromKalshiTicker(parsed, book));
