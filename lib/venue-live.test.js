...
(async () => {
  ...
  const asks = await loadPolymarketAsks([
    ...
    fetchFn: async (_url, init) => {
      postedSide = JSON.parse(init.body)[0].side;
      return {
        ok: true,
        status: 200,
        text: async () => JSON.stringify({ 'tok-away': { BUY: '0.15', SELL: '0.16' } }),
      };
    },
  });
  assert.equal(postedSide, 'SELL');
  assert.equal(asks[0].odds, 0.16, 'CLOB SELL is the ask; BUY is the bid and must not paint');
