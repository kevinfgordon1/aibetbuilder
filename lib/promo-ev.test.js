...
// ── Formula strings still present in App.jsx (CJS copy must not drift)
{
  ...
  assert.ok(bookKeys.includes("betanysports"), "ALL_BOOKS missing betanysports");
  assert.equal(TRUSTED_BOOK_KEYS.has("betanysports"), false, "betanysports must stay out of TRUSTED_BOOK_KEYS");
  assert.ok(!bookKeys.includes("betopenly"), "BetOpenly must not be an EV / Promo Builder book");
  assert.equal(TRUSTED_BOOK_KEYS.has("betopenly"), false, "betopenly must stay out of TRUSTED_BOOK_KEYS");
  for (const key of ["bovada", "mybookieag", "betonlineag"]) {
  ...
  assert.ok(!app.includes('key: "betopenly"'), "App.jsx must not offer BetOpenly as an EV book");
}
...
// ── BetAnything: selectable EV book, never trusted opp. BetOpenly is dropped.
{
  const future = new Date(Date.now() + 36 * 60 * 60 * 1000).toISOString();
  const games = [{
    commence_time: future,
  ...
  assert.equal(boYankees, undefined, "BetOpenly must not produce EV / Promo Builder legs");
  assert.equal(data.moneylines[0].bookOdds.betopenly, undefined);
...
// ── Bovada / MyBookie / BetOnline are trusted opponent/fair prices
{
  const future = new Date(Date.now() + 36 * 60 * 60 * 1000).toISOString();
  const games = [{
    commence_time: future,
  ...
  const dkYankees = buildAllLegsForBook(data, "draftkings").find(l => l.name === "Yankees ML");
  assert.ok(dkYankees);
  assert.equal(dkYankees.bestOpp, 115);
  assert.equal(dkYankees.bestOppBook, "betonlineag");
