...
// ── formatOdds: +plus / −minus, never "+-105"
{
  assert.equal(formatOdds(120), "+120");
  assert.equal(formatOdds(-105), "-105");
  assert.equal(formatOdds(-101), "-101");
  assert.equal(formatOdds("+120"), "+120");
  assert.equal(formatOdds("-105"), "-105");
  assert.equal(formatOdds("+-105"), "-105");
  for (const sample of [120, -105, -101, "+120", "-101", "+-105"]) {
    assert.ok(!formatOdds(sample).includes("+-"), `no +- prefix for ${sample}`);
  }
}

// ── Screenshot: Pinnacle -105, Win $99 / $100 stake → boosted -101
{
  const boostedOdds = decimalToAmerican(1 + 99 / 100);
  assert.equal(boostedOdds, -101);
  assert.equal(formatOdds(boostedOdds), "-101");
  assert.notEqual(formatOdds(boostedOdds), "+-105");
  assert.notEqual(formatOdds(-105), "+-105");
}
...
// ── Formula strings still present in App.jsx (CJS copy must not drift)
{
  ...
  assert.match(app, /numLegs < MAX_PROMO_LEGS/);
  assert.match(app, /Math\.min\(MAX_PROMO_LEGS, n \+ 1\)/);
  ...
  assert.equal(MAX_PROMO_LEGS, 10);
  assert.equal(GROW_FROM_3_SEEDS, 50);
  ...
  for (const key of ["bovada", "mybookieag", "betonlineag", "bookmaker", "pinnacle", "betus", "underdog_predict"]) {
    assert.ok(bookKeys.includes(key), `ALL_BOOKS missing ${key}`);
  ...
  assert.ok(!/fanatics|crypto/i.test(ALL_BOOKS.find(b => b.key === "underdog_predict")?.label || ""));
  assert.equal(ALL_BOOKS.find(b => b.key === "bookmaker")?.label, "Bookmaker");
  assert.equal(bookKeys.includes("betcris"), false, "BetCris stays off Promo ALL_BOOKS (same shop as Bookmaker)");
  assert.equal(TRUSTED_BOOK_KEYS.has("betcris"), false, "BetCris stays out of Promo TRUSTED_BOOK_KEYS");
  assert.equal(ALL_BOOKS.find(b => b.key === "pinnacle")?.label, "Pinnacle");
  assert.match(app, /key: "pinnacle"[\s\S]*?label: "Pinnacle"/);
  assert.equal(bookKeys.includes("courtside"), false, "Courtside stays off Promo ALL_BOOKS");
  assert.equal(TRUSTED_BOOK_KEYS.has("courtside"), false, "Courtside stays out of Promo TRUSTED_BOOK_KEYS");
  assert.doesNotMatch(app, /key: "courtside"/);
  assert.equal(ALL_BOOKS.find(b => b.key === "betparx")?.label, "betPARX");
  assert.equal(ALL_BOOKS.find(b => b.key === "ballybet")?.label, "Bally Bet");
  assert.match(app, /key: "betparx"[\s\S]*?label: "betPARX"/);
  assert.match(app, /key: "ballybet"[\s\S]*?label: "Bally Bet"/);
  assert.equal(TRUSTED_BOOK_KEYS.has("hardrockbet_fl"), false);
  ...
  assert.ok(appTrusted[1].includes("betus"));
  ...
  assert.match(app, /formatTrueOddsWithBlend/);
  assert.match(app, /labelBestOppLine/);
  assert.doesNotMatch(app, /TRUE_ODDS_INVERSE_HINT/);
  assert.doesNotMatch(app, /True Odds is the inverse of this best opp/);
  const sublineFn = app.slice(app.indexOf("function PromoTrueOddsSubline"), app.indexOf("function PromoExpandedLegsTable"));
  assert.match(sublineFn, /labelBestOppLine\(/);
  assert.doesNotMatch(sublineFn, /\{line\.text\}/);
  assert.doesNotMatch(sublineFn, /True Odds is the inverse of this best opp/);
  assert.ok(app.includes("<PromoTrueOddsSubline"), "cards and expanded LEG cells share the labeled subline");
  assert.match(app, /formatDepthTrail/);
  assert.match(app, /blendAskLadderToPayout|usePromoDepthBlend|overlayParlayMetrics/);
  const rankSrc = fs.readFileSync(path.join(__dirname, "../src/promoListRank.js"), "utf8");
  assert.match(rankSrc, /preferCompletePmHedge/);
  assert.match(app, /LOW_LIQUIDITY_LABEL|Low liquidity/);
  assert.match(app, /quotedOppAmerican|bestOppQuoted/);
  assert.match(app, /trueOppAmerican/);
  assert.match(app, /\$500 payout blend/);
  assert.match(app, /rankPromoPicks/);
  assert.match(rankSrc, /rankPicksAfterOppGuard/);
  assert.match(app, /resolveOppWithSideGuard/);
  assert.match(app, /formatSignedEvMoney/);
  assert.match(app, /formatSignedEvPct/);
  assert.match(app, /formatPromoTotalBookOdds\(boostedOdds\)/);
  assert.doesNotMatch(app, /\+\{p\.parlayOdds\}/);
  assert.doesNotMatch(app, /\+\$\{view\.ev\.toFixed/);
  assert.doesNotMatch(app, /\+\{\(view\.ev \/ stake \* 100\)/);
  const fetchOdds = fs.readFileSync(path.join(__dirname, "../lib/odds-fetch-job.js"), "utf8");
  assert.match(fetchOdds, /includeBetLimits=true/);
  assert.match(fetchOdds, /top-of-book|TOP-OF-BOOK|top of book/i);
  const bookDepth = fs.readFileSync(path.join(__dirname, "../api/book-depth.js"), "utf8");
  assert.match(bookDepth, /Never calls The Odds API/);
}
...
// ── BetUS is a trusted opponent/fair price (soft retail, same set as DK / Pinnacle)
{
  const future = new Date(Date.now() + 36 * 60 * 60 * 1000).toISOString();
  const games = [{
    commence_time: future,
    ...
    bookmakers: [
      ...
      },
      {
        key: "betus",
        markets: [
          { key: "h2h", outcomes: [{ name: "Yankees", price: -110 }, { name: "Red Sox", price: 125 }] },
        ],
      },
    ],
  }];
  ...
  const dkYankees = buildAllLegsForBook(data, "draftkings").find(l => l.name === "Yankees ML");
  assert.ok(dkYankees);
  assert.equal(dkYankees.bestOpp, 125);
  assert.equal(dkYankees.bestOppBook, "betus");

  const betusYankees = buildAllLegsForBook(data, "betus").find(l => l.name === "Yankees ML");
  assert.ok(betusYankees);
  assert.equal(betusYankees.dk, -110);
  assert.equal(betusYankees.bookKey, "betus");
}
...
// ── grow-from-3: 9 and 10 legs succeed; 11 is over the stepper cap
{
  const future = new Date(Date.now() + 36 * 60 * 60 * 1000).toISOString();
  ...
  for (const n of [9, 10]) {
    const top = findTopParlays(legs, n, 30, 100, 5);
    assert.ok(top.length >= 1, `${n}-leg parlay should be returned`);
    assert.equal(top[0].legs.length, n);
    assert.equal(new Set(top[0].legs.map(l => l.game)).size, n, `${n}-leg must stay one pick per game`);
  }
  assert.deepEqual(findTopParlays(legs, 11, 30, 100, 5), []);
}

// ── 9–10 legs stay on the 50-seed beam (no C(n,9) / C(n,10))
{
  const future = new Date(Date.now() + 36 * 60 * 60 * 1000).toISOString();
  ...
  const top8 = findTopParlays(legs, 8, 25, 100, 8);
  const top9 = findTopParlays(legs, 9, 25, 100, 8);
  const top10 = findTopParlays(legs, 10, 25, 100, 8);
  const elapsed = Date.now() - started;
  assert.equal(top8[0].legs.length, 8);
  assert.equal(top9[0].legs.length, 9);
  assert.equal(top10[0].legs.length, 10);
  assert.ok(elapsed < 2000, `8–10 leg grow on 40 games took ${elapsed}ms`);
}

// ── Combo Locks / Kalshi combo / book-depth caps are separate from the stepper
{
  const probe = fs.readFileSync(path.join(__dirname, "../api/combo-probe-lib.js"), "utf8");
  assert.match(probe, /out\.length > 12/);
  assert.match(probe, /Too many legs \(max 12\)/);
  const depth = fs.readFileSync(path.join(__dirname, "../lib/book-depth.js"), "utf8");
  assert.match(depth, /const MAX_LEGS = 8;/);
  const clientDepth = fs.readFileSync(path.join(__dirname, "../src/promoBookDepth.js"), "utf8");
  assert.match(clientDepth, /DEPTH_BATCH = 8/);
}
...
// ── Soccer 3-way: Home Yes pairs with Home No, never Away Yes
{
  const future = new Date(Date.now() + 36 * 60 * 60 * 1000).toISOString();
  const games = [{
    commence_time: future,
      ...
      },
      {
        ...
        markets: [
          ...
          },
          {
            key: "h2h_lay",
            outcomes: [
              { name: "Chelsea", price: -150, bet_limit: 400 },
              { name: "Draw", price: -160, bet_limit: 350 },
              { name: "Arsenal", price: 155, bet_limit: 900 },
            ],
          },
        ],
      },
  ...
  const data = transformOddsData(games, "soccer_epl");
  const g = data.moneylines[0];
  assert.equal(g.is_three_way, true);
  assert.equal(g.bookOdds.draftkings.ml_draw, 240);
  assert.equal(g.best_home_no, 155);
  assert.equal(g.best_home_no_book, "kalshi");
  assert.notEqual(g.best_home_no, g.best_away, "do not use Away Yes as Home inverse");

  const dkLegs = buildAllLegsForBook(data, "draftkings");
  const home = dkLegs.find((l) => l.name === "Arsenal ML");
  const away = dkLegs.find((l) => l.name === "Chelsea ML");
  const draw = dkLegs.find((l) => l.name === "Draw");
  assert.ok(home && away && draw);
  assert.equal(home.bestOpp, 155);
  assert.equal(home.bestOppBook, "kalshi");
  assert.equal(home.bestOppName, "Arsenal ML No");
  assert.equal(home.soccerBinary, "home");
  assert.equal(away.bestOpp, -150);
  assert.equal(away.bestOppName, "Chelsea ML No");
  assert.equal(draw.bestOpp, -160);
  assert.equal(draw.bestOppName, "Draw No");

  const nfl = transformOddsData([{
    commence_time: future,
    away_team: "Chiefs",
    home_team: "Bills",
    bookmakers: [{
      key: "draftkings",
      markets: [{ key: "h2h", outcomes: [{ name: "Chiefs", price: 110 }, { name: "Bills", price: -130 }] }],
    }, {
      key: "kalshi",
      markets: [{ key: "h2h", outcomes: [{ name: "Chiefs", price: 105 }, { name: "Bills", price: 120 }] }],
    }],
  }], "americanfootball_nfl");
  const nflLegs = buildAllLegsForBook(nfl, "draftkings");
  const chiefs = nflLegs.find((l) => l.name === "Chiefs ML");
  assert.equal(chiefs.bestOpp, 120, "2-way sports still use the other team's Yes");
  assert.equal(chiefs.bestOppName, "Bills ML");
}

// ── Soccer No: Kalshi Yes-only + exchange h2h_lay (production Odds API shape)
{
  assert.equal(TRUSTED_BOOK_KEYS.has("betfair_ex_eu"), false, "exchange lays stay out of TRUSTED_BOOK_KEYS");
  assert.equal(TRUSTED_BOOK_KEYS.has("matchbook"), false);
  const future = new Date(Date.now() + 36 * 60 * 60 * 1000).toISOString();
  const games = [{
    commence_time: future,
    away_team: "Chelsea",
  ...
  const data = transformOddsData(games, "soccer_epl");
  const g = data.moneylines[0];
  assert.equal(g.best_home_no, -138);
  assert.equal(g.best_home_no_book, "betfair_ex_eu");
  assert.equal(g.best_home_no_size, 14);
  assert.equal(g.best_away_no, -245);
  assert.equal(g.best_draw_no, -250);
  assert.notEqual(g.best_home_no, g.best_away, "do not use Away Yes as Home inverse");
  ...
  const away = dkLegs.find((l) => l.name === "Chelsea ML");
  const draw = dkLegs.find((l) => l.name === "Draw");
  assert.ok(home && away && draw, "DK soccer ML legs exist via last-resort exchange No");
  assert.equal(home.bestOpp, -138);
  assert.equal(home.bestOppBook, "betfair_ex_eu");
  assert.equal(home.bestOppName, "Arsenal ML No");
  assert.equal(away.bestOpp, -245);
  assert.equal(away.bestOppName, "Chelsea ML No");
  assert.equal(draw.bestOpp, -250);
  assert.equal(draw.bestOppName, "Draw No");

  const bovadaLegs = buildAllLegsForBook(data, "bovada");
  const over = bovadaLegs.find((l) => l.market === "TOT" && l.name.includes("o2.5"));
  const under = bovadaLegs.find((l) => l.market === "TOT" && l.name.includes("u2.5"));
  assert.ok(over && under, "soccer totals still pair via same-book opposite");
  assert.equal(over.bestOpp, -110);
  assert.equal(under.bestOpp, -110);
  ...
  const nfl = transformOddsData([{
    ...
    bookmakers: [{
      ...
      markets: [{
        key: "h2h",
        outcomes: [{ name: "Chiefs", price: 200 }, { name: "Bills", price: 300 }],
      }],
    }],
  }], "americanfootball_nfl");
  const nflLegs = buildAllLegsForBook(nfl, "draftkings");
  const chiefs = nflLegs.find((l) => l.name === "Chiefs ML");
  assert.equal(chiefs.bestOpp, -130, "non-soccer inverse ignores untrusted exchange Yes");
  assert.equal(chiefs.bestOppBook, "draftkings");
  assert.equal(chiefs.bestOppName, "Bills ML");
}

// ── Iowa screenshot: inverted Kalshi opp cannot become +2042 true vs Fanatics −20000
{
  const future = new Date(Date.now() + 36 * 60 * 60 * 1000).toISOString();
  const games = [{
    commence_time: future,
  ...
  const data = transformOddsData(games, "americanfootball_ncaaf");
  const iowa = buildAllLegsForBook(data, "fanatics").find((l) => l.name === "Iowa Hawkeyes ML");
  assert.ok(iowa);
  assert.notEqual(iowa.bestOppBook, "kalshi");
  assert.ok(iowa.bestOpp > 0);
  assert.ok(ourTrueProb(iowa.bestOpp) > 0.9);
}

// ── Inverse bet stays: Dbacks Caesars −165, best opp Underdog +163 → True Odds −163
{
  const future = new Date(Date.now() + 36 * 60 * 60 * 1000).toISOString();
  const games = [{
    commence_time: future,
    ...
    bookmakers: [
      {
        ...
        markets: [{
          key: "h2h",
          outcomes: [
            { name: "Arizona Diamondbacks", price: -165 },
            { name: "Colorado Rockies", price: 145 },
          ],
        }],
      },
      {
        ...
        markets: [{
          key: "h2h",
          outcomes: [
            { name: "Arizona Diamondbacks", price: -180, predictionAmerican: -180 },
            { name: "Colorado Rockies", price: 163, predictionAmerican: 163 },
          ],
        }],
      },
      {
        ...
        markets: [{
          key: "h2h",
          outcomes: [
            { name: "Arizona Diamondbacks", price: -170 },
            { name: "Colorado Rockies", price: 150 },
          ],
        }],
      },
    ],
  ...
  const data = transformOddsData(games, "baseball_mlb");
  const dbacks = buildAllLegsForBook(data, "williamhill_us").find((l) => l.name === "Arizona Diamondbacks ML");
  assert.ok(dbacks);
  assert.equal(dbacks.dk, -165);
  assert.equal(dbacks.bestOpp, 163);
  assert.equal(dbacks.bestOppBook, "underdog_predict");
  assert.equal(dbacks.bestOppName, "Colorado Rockies ML");
  const r = calcParlayEV([dbacks], 0, 100);
  assert.equal(probToAmerican(ourTrueProb(163)), -163);
  assert.equal(probToAmerican(r.combinedProb), -163);
  assert.notEqual(probToAmerican(r.combinedProb), 163, "same-side +163 is not True Odds");
  assert.notEqual(probToAmerican(r.combinedProb), -180, "Underdog same-side phone is not the fair");
  assert.ok(r.ev < 0, "Caesars −165 vs fair −163 is a small negative EV");
}

// ── Pinnacle-style sportsbook still uses complement of best opp
{
  const future = new Date(Date.now() + 36 * 60 * 60 * 1000).toISOString();
  const games = [{
    commence_time: future,
    ...
    bookmakers: [
      {
        ...
        markets: [{
          ...
          outcomes: [
            { name: "Arizona Diamondbacks", price: -165 },
            { name: "Colorado Rockies", price: 140 },
          ],
        }],
      },
      {
        ...
        markets: [{
          key: "h2h",
          outcomes: [
            { name: "Arizona Diamondbacks", price: -155 },
            { name: "Colorado Rockies", price: 155 },
          ],
        }],
      },
    ],
  ...
  const data = transformOddsData(games, "baseball_mlb");
  const dbacks = buildAllLegsForBook(data, "williamhill_us").find((l) => l.name === "Arizona Diamondbacks ML");
  assert.equal(dbacks.bestOpp, 155);
  assert.equal(dbacks.bestOppBook, "pinnacle");
  assert.equal(probToAmerican(calcParlayEV([dbacks], 0, 100).combinedProb), -155);
  assert.equal(probToAmerican(ourTrueProb(dbacks.bestOpp)), -155);
}
