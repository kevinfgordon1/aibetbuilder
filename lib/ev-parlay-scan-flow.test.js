...
async function run() {
  ...
  assert.equal(ODDS_CACHE_TIMEOUT_MS, 25000);
  assert.ok(ODDS_CACHE_TIMEOUT_MS >= 20000 && ODDS_CACHE_TIMEOUT_MS <= 30000);
  assert.ok(ODDS_CACHE_TIMEOUT_MS < 60000, "odds_cache cap must stay under maxDuration: 60");
...

  {
    ...
    try {
      ...
      assert.ok(Date.now() - t0 < 500, "MLB-only timeout must abort, not wait maxDuration");
      assert.equal(out.ok, false);
      assert.equal(out.status, 504);
      assert.equal(out.timedOut, true);
      assert.deepEqual(out.loadedSports, []);
      assert.deepEqual(out.timedOutSports, ["baseball_mlb"]);
      assert.equal(out.message, "odds_cache timed out after 40ms (baseball_mlb)");
    } finally {
      console.log = origLog;
    }
  }

  {
    ...
    try {
      ...
      assert.match(String(out.message), /timed out after 40ms \(baseball_mlb, basketball_nba\)/);
    } finally {
      console.log = origLog;
    }
  }
  ...
  // ── handler source: send-first flow, no KayGo token; Vercel cron gone
  {
    ...
    assert.ok(jobSrc.includes("ODDS_CACHE_TIMEOUT_MS"));
    ...
    assert.match(flowSrc, /OPTIONAL_TABLE_TIMEOUT_MS = 2000/);
    assert.match(flowSrc, /ODDS_CACHE_TIMEOUT_MS = 25000/);
    ...
    assert.ok(!jobSrc.includes("EV_ALERT_TELEGRAM_BOT_TOKEN"));
    ...
    assert.ok(vercel.crons.some(c => c.path === "/api/fetch-odds"));
    assert.ok(vercel.functions["api/scan-ev-parlays.js"]);
  }
}
