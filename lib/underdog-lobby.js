...
// than 60 minutes (UNDERDOG_STALE_MS) is omitted; a missing timestamp is
// kept. Staleness is per side, so Central Michigan can stay when Miami on
// the same game is frozen. Two-way moneylines whose implied probabilities
// sum outside 0.80–1.22 are omitted (Akron −527 / Central Michigan −715).
// A normal −110 / −110 pair stays. Phone PE only.
...
const { sanitizeUnderdogPhoneLines } = require('./underdog-freshness');
...
function gamesFromContentLines(body, fallbackSportId, now = Date.now()) {
  ...
  return [...byMatch.values()].map((game) => ({
    ...game,
    lines: sanitizeUnderdogPhoneLines(game.lines, now),
