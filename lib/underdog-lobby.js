...
// odds.prediction quote is omitted. Phone PE only.
...
function gamesForMarket(body, sportId, market) {
...
async function fetchMarketLines(fetchFn, cfg, sportId, filterId, market) {
  const res = await fetchJson(fetchFn, contentLinesUrl(cfg, sportId, filterId), requestHeaders(cfg));
  const games = res.ok && res.body ? gamesForMarket(res.body, sportId, market) : [];
  return { res, games };
}

async function fetchCoreMarket(fetchFn, cfg, sportId, spec, scaffoldBody) {
  ...
  const take = async (filterId) => {
    if (!filterId) return [];
    attempts += 1;
    const result = await fetchMarketLines(fetchFn, cfg, sportId, filterId, spec.market);
...
async function fetchIndexedLines(fetchFn, cfg, sportId) {
  ...
  const fetched = await mapPool(CORE_MARKETS, CORE_MARKETS.length, (spec) => (
    fetchCoreMarket(fetchFn, cfg, sportId, spec, scaffold.body)
  ));
  let saw404 = 0;
  let attempts = 0;
  const lists = [];
  ...
  return { games: mergeGames(lists), saw404, attempts };
}
...
async function fetchUnderdogPhone(deps) {
  ...
  const cacheKey = `${cfg.base}|${cfg.product}|${cfg.stateConfigId}|${cfg.productExperienceId}|h2h-spreads-totals`;
