...
// Pure module — no Supabase / Telegram I/O. Used by the Railway worker and
// leftover /api/scan-ev-parlays HTTP handler.
...
// Promo Builder / +EV Bets keep ALL_BOOKS (including BetOpenly). Telegram
// EV-parlay alerts omit books whose prices are too noisy to ticket.
const EV_ALERT_EXCLUDE_BOOK_KEYS = new Set(["betopenly"]);
const EV_ALERT_BOOKS = ALL_BOOKS.filter(b => !EV_ALERT_EXCLUDE_BOOK_KEYS.has(b.key));

const STAKE = 100;
const BOOST_PCT = 0;
const NUM_LEGS = 3;
const MIN_EV_PCT = 3;
// Telegram scan is MLB-only, Promo Builder "Next 24h" window.
const SCAN_SPORT_KEYS = ["baseball_mlb"];
const SCAN_DATE_RANGE = "24h";
...
function scanBooksForEvParlays(oddsData, opts = {}) {
  ...
  const books = opts.books || EV_ALERT_BOOKS;
  const sportFilter = opts.sportFilter ?? SCAN_SPORT_KEYS;
  const dateRange = opts.dateRange ?? SCAN_DATE_RANGE;
  const t0 = Date.now();
  const globalTop = [];
  const stats = [];
  let comboCount = 0;
  ...
  for (const book of books) {
    ...
    const rawLegs = buildAllLegsForBook(oddsData, book.key, sportFilter, null, dateRange, null, { underdogCash: true });
