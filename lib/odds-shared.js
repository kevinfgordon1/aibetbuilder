...
const { applyUnderdogPredictFee } = require('./underdog-predict-fee');
...
// underdog_predict display is the phone odds.prediction quote. Do not
// fee-adjust the sticker (UDX 0.072×p×(1−p) would turn Browns +331 into
// +308). New Odds Board does not paint Betstamp book 196. This function
// leaves an existing bookmaker price unchanged. Promo uses the joined
// odds.prediction american and omits the leg when that quote is missing.
// Do not apply applyUnderdogPredictFee here. Helper stays exported.
// ─────────────────────────────────────────────────────────────────────────
  ...
  return sportData.map(game => {
    ...
    return {
      ...
      bookmakers: game.bookmakers.map(bookmaker => {
        ...
        } else if (bookmaker.key === 'underdog_predict') {
          return bookmaker;
        } else {
          return bookmaker;
        }
...
module.exports = { supabase, applyBookAdjustments, applyUnderdogPredictFee };
