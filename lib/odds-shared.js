...
// + underdog_predict (UDX fee: rate × p × (1−p), added to cost). Underdog is
// Betstamp-only today; the Promo overlay applies the same haircut if a
// bookmaker with this key appears.
// ─────────────────────────────────────────────────────────────────────────
    ...
    return {
      ...
      bookmakers: game.bookmakers.map(bookmaker => {
        ...
        if (bookmaker.key === 'kalshi') {
          ...
          adjustFn = applyUnderdogPredictFee;
        } else {
          return bookmaker;
        }
        return {
