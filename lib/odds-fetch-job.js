...
const { SOCCER_SPORT_KEYS, isSoccerSport } = require('./soccer-pairing');
...
// Soccer stays featured-only (h2h 3-way + totals). Do not pull alt props.
const EVENT_MARKETS = Object.fromEntries(
  FEATURED_SPORTS.filter((sport) => !isSoccerSport(sport)).map((sport) => [sport, ALT_MARKETS]),
);
...
function parseRequestedSports(raw, allowed = FEATURED_SPORTS) {
  ...
  // UI alias "soccer" and a lone EPL/MLS key both mean both Odds API soccer keys.
  // "soccer" is not an Odds API sport key.
  if (wanted.has('soccer') || SOCCER_SPORT_KEYS.some((k) => wanted.has(k))) {
    for (const k of SOCCER_SPORT_KEYS) wanted.add(k);
  }
  wanted.delete('soccer');
...
function featuredMarketsParam(sport) {
  // Soccer 3-way: h2h is Home/Draw/Away Yes. h2h_lay is exchange lay
  // (converted to same-binary No) or a PM No when Odds API exposes one.
  // Other sports stay 2-way h2h.
  if (isSoccerSport(sport)) return 'h2h,h2h_lay,spreads,totals';
...
function featuredOddsUrl(sport, apiKey) {
  ...
  return `https://api.the-odds-api.com/v4/sports/${sport}/odds/?apiKey=${apiKey}&regions=us,us2,us_ex,eu&markets=${featuredMarketsParam(sport)}&oddsFormat=american&includeBetLimits=true`;
}
