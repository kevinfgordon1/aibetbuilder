...
// Shared Odds API + odds_cache job used by /api/fetch-odds (cron) and
// /api/odds-cache (Promo selected-sports read, cache then live fallback).
...
const CACHE_READ_TIMEOUT_MS = 4000;
const UPSERT_TIMEOUT_MS = 8000;
const ODDS_CACHE_COLUMNS = 'sport,data,fetched_at';
const EVENT_ODDS_CACHE_COLUMNS = 'event_id,sport,commence_time,home_team,away_team,data,markets,fetched_at';
...
async function readCachedSport(supabaseClient, sport, { timeoutMs = CACHE_READ_TIMEOUT_MS } = {}) {
  try {
    const result = await withTimeout(
      (signal) => applyAbortSignal(
        supabaseClient.from('odds_cache').select(ODDS_CACHE_COLUMNS).eq('sport', sport).maybeSingle(),
        signal,
      ),
...
async function runFetchOddsJob({
  ...
  // with PostgREST's statement timeout and blocking Promo reads.
  ...
  return { success: true, results, featured };
}

async function readCachedEvents(supabaseClient, sports, eventSince, { timeoutMs = CACHE_READ_TIMEOUT_MS } = {}) {
  try {
    const result = await withTimeout(
      (signal) => {
        let q = supabaseClient.from('event_odds_cache').select(EVENT_ODDS_CACHE_COLUMNS).in('sport', sports);
        if (eventSince) q = q.gte('commence_time', eventSince);
        return applyAbortSignal(q, signal);
      },
...
async function loadSelectedSportsOdds({
  sports = FEATURED_SPORTS,
  eventSince = null,
  apiKey,
  fetchImpl,
  supabaseClient,
  applyBookAdjustments,
  cacheReadTimeoutMs = CACHE_READ_TIMEOUT_MS,
} = {}) {
  const featured = [];
  const errors = [];

  for (const sport of sports || []) {
    const cached = await readCachedSport(supabaseClient, sport, { timeoutMs: cacheReadTimeoutMs });
    if (cached && cached.data != null) {
      featured.push(cached);
      continue;
    }
    const live = await pullFeaturedSport(sport, { apiKey, fetchImpl, applyBookAdjustments });
    if (!live.data) {
      errors.push({ sport, error: (live.error && live.error.message) || 'unavailable' });
      continue;
    }
    try { await upsertFeatured(supabaseClient, sport, live.data); } catch (_) { /* still return live */ }
    featured.push({
      sport,
      data: live.data,
      fetched_at: new Date().toISOString(),
  ...
  const events = await readCachedEvents(supabaseClient, sports, eventSince);
  return { featured, events, errors };
}
