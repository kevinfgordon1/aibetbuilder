...
assert.deepEqual(b.parseBookIds('400'), b.DEFAULT_BOOK_IDS, '400-only falls back — never forward BetMGM');
assert.deepEqual(b.parseBookIds('100,200,400'), [100, 200], 'strip BetMGM 400 from mixed book_ids');
assert.ok(b.TRIAL_BOOK_IDS.includes(196));
assert.ok(!b.TRIAL_BOOK_IDS.includes(400), 'BetMGM 400 is not forwarded to Betstamp');
assert.ok(b.BLOCKED_UPSTREAM_BOOK_IDS.includes(400));
assert.equal(b.BETMGM_BOOK_ID, 400);
assert.ok(!b.DEFAULT_BOOK_IDS.includes(196), 'anon default fetches omit Underdog Predict');
assert.ok(!b.DEFAULT_BOOK_IDS.includes(400), 'anon default fetches omit BetMGM 400 (403s the slate)');
...
assert.ok(b.parseBookIds('999').every((id) => id !== 196));
...
assert.match(b.redact({ code: 'forbidden', book_id: 400 }), /forbidden/);
assert.doesNotMatch(b.redact({ code: 'forbidden', book_id: 400 }), /\[object Object\]/);
