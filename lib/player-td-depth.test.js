...
(async () => {
  ...
  assert.equal(hits, 1, 'stops at the first 429');
  ...
  // Scan pool is pre-blended so ranking uses the same fair as the card.
  const [pre] = blend.preBlendStoredLadderLegs([leg], {});
  assert.equal(pre.bestOpp, walked.bestOpp);
  assert.equal(pre.bestOppQuoted, leg.bestOpp);
  const plain = { name: 'x', bestOpp: -120, bestOppBook: 'kalshi' };
  assert.equal(blend.preBlendStoredLadderLegs([plain], {})[0], plain, 'legs without a stored ladder are untouched');
