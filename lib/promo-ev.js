...
function buildAllLegsForBook(data, book, sportFilter = null, minLegOdds = null, dateRange = "any", maxLegOdds = null, opts = null) {
  ...
  // quote omits that leg. A stale Underdog moneyline (updated_at older than
  // UNDERDOG_STALE_MS, 1 hour) is not a Promo candidate. Player touchdown
  // offers use the board clock (UNDERDOG_BOARD_OMIT_MS, 24 hours) so Promo
  // sees the same Underdog coverage as Player Props. Older than that is
  // omitted.
