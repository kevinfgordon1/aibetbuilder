...
// Promo Underdog is prediction-only. +252 sticker must not become +244.
{
  ...
  const series = stampUnderdogPredictionLegs([
    ...
    { bookKey: 'underdog_predict', dk: 104, name: 'San Diego Padres/Los Angeles Dodgers o8.5', game: 'San Diego Padres @ Los Angeles Dodgers', commence_time: tonight },
  ], {
    run_lines: [
    ...
    totals: [
      { book: 'underdog_predict', away: 'San Diego Padres', home: 'Los Angeles Dodgers', line: 8.5, commence_time: tonight, over_prediction: 104 },
      { book: 'underdog_predict', away: 'San Diego Padres', home: 'Los Angeles Dodgers', line: 8.5, commence_time: tomorrow, over_prediction: -110 },
    ],
  });
  const seriesPriced = applyUnderdogCashLegPrices(series, true);
  const tonightDodgers = seriesPriced.find((l) => l.commence_time === tonight && l.name === 'Los Angeles Dodgers -1.5');
  const tomorrowDodgers = seriesPriced.find((l) => l.commence_time === tomorrow && l.name === 'Los Angeles Dodgers -1.5');
  assert.equal(tonightDodgers.dk, -109, 'stamp must not replace tonight -109 with tomorrow +122');
  assert.equal(tonightDodgers.predictionAmerican, -109);
  assert.equal(tomorrowDodgers.dk, 122);
  assert.equal(seriesPriced.find((l) => l.name.includes('o8.5')).dk, 104);
}
