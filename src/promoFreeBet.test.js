...
// ── 1-leg without an opposite is not a fake lock (lock is null, not {valid:false})
{
  ...
  // Card render used to throw on p.lock.guaranteedCash when valid:false.
  assert.equal(unlocked.lock?.guaranteedCash, undefined);
  assert.equal((unlocked.lock?.guaranteedCash ?? 0).toFixed(2), "0.00");
}
