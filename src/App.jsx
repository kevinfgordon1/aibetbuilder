  ...
  ALL_BOOKS.forEach(book => {
    if (data.moneylines) {
      data.moneylines.forEach(g => {
        ...
        const ak = `${g.commence_time || ""}\0${g.away}@${g.home}_ML_away_${book.key}`;
        const hk = `${g.commence_time || ""}\0${g.away}@${g.home}_ML_home_${book.key}`;
    ...
    if (data.run_lines) {
      data.run_lines.forEach(g => {
        ...
        const homeOdds = g.home_odds;
        if (underdogPairIncomplete(book.key, awayOdds, homeOdds)) return;
        const ak = `${g.commence_time || ""}\0${g.away}@${g.home}_SPR_away_${g.away_line}_${book.key}`;
        const hk = `${g.commence_time || ""}\0${g.away}@${g.home}_SPR_home_${g.home_line}_${book.key}`;
    ...
    if (data.totals) {
      data.totals.forEach(g => {
        ...
        const ok = `${g.commence_time || ""}\0${g.away}@${g.home}_TOT_over_${g.line}_${book.key}`;
        const uk = `${g.commence_time || ""}\0${g.away}@${g.home}_TOT_under_${g.line}_${book.key}`;
    ...
    if (data.team_totals) {
      data.team_totals.forEach(g => {
        ...
        const underOdds = g.under_odds;
        if (underdogPairIncomplete(book.key, overOdds, underOdds)) return;
        const ok = `${g.commence_time || ""}\0${g.away}@${g.home}_TT_${g.team}_o_${g.line}_${book.key}`;
        const uk = `${g.commence_time || ""}\0${g.away}@${g.home}_TT_${g.team}_u_${g.line}_${book.key}`;
