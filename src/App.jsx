...
import { applyUnderdogCashLegPrices, underdogCashOfferAmerican } from "./underdogPredictFee.js";
...
function pushSoccerMlLegs(legs, g, bookKey, { seen, minLegOdds, maxLegOdds, underdogCash } = {}) {
  for (const side of SOCCER_ML_SIDES) {
    ...
    const offered = underdogCashOfferAmerican(bookKey, odds, underdogCash);
...
function buildAllLegsForBook(data, book, sportFilter = null, minLegOdds = null, dateRange = "any", maxLegOdds = null, opts = null) {
  ...
  if (data.moneylines) {
    data.moneylines.forEach(g => {
      ...
      if (passesOddsBounds(underdogCashOfferAmerican(book, awayOdds, underdogCash), minLegOdds, maxLegOdds))
      ...
      if (passesOddsBounds(underdogCashOfferAmerican(book, homeOdds, underdogCash), minLegOdds, maxLegOdds))
  ...
  if (data.team_totals) {
    ...
    data.team_totals.forEach(g => {
      ...
      if (!seen.has(ok) && passesOddsBounds(underdogCashOfferAmerican(book, overOdds, underdogCash), minLegOdds, maxLegOdds)) { seen.add(ok); legs.push(assignBookUpdatedAt({ name: `${g.team} TT o${g.line}`, dk: overOdds, market: "TT", game: `${g.away} @ ${g.home}`, commence_time: g.commence_time, sport: g.sport, bookKey: book, bestOppName: g.bestOppName_over, isAlt: !!g.is_alt, ...resolveOpp({ trustedOpp: g.bestOpp_over, trustedBook: g.bestOpp_over_book, trustedCount: g.bestOppCount_over, trustedSize: g.bestOpp_over_size, sameBookOpp: underOdds, sameBookKey: book, sameBookSize: g.under_size, bookOdds: overOdds }) }, g.over_updatedAt)); }
      if (!seen.has(uk) && passesOddsBounds(underdogCashOfferAmerican(book, underOdds, underdogCash), minLegOdds, maxLegOdds)) { seen.add(uk); legs.push(assignBookUpdatedAt({ name: `${g.team} TT u${g.line}`, dk: underOdds, market: "TT", game: `${g.away} @ ${g.home}`, commence_time: g.commence_time, sport: g.sport, bookKey: book, bestOppName: g.bestOppName_under, isAlt: !!g.is_alt, ...resolveOpp({ trustedOpp: g.bestOpp_under, trustedBook: g.bestOpp_under_book, trustedCount: g.bestOppCount_under, trustedSize: g.bestOpp_under_size, sameBookOpp: overOdds, sameBookKey: book, sameBookSize: g.over_size, bookOdds: underOdds }) }, g.under_updatedAt)); }
    });
  }

  const priced = applyUnderdogCashLegPrices(legs.filter((l) => l.bestOpp != null), underdogCash);
  ...
  ALL_BOOKS.forEach(book => {
    ...
    if (data.team_totals) {
      data.team_totals.forEach(g => {
        ...
        if (!seen.has(uk)) { seen.add(uk); legs.push(assignBookUpdatedAt({ name: `${g.team} TT u${g.line}`, dk: underOdds, market: "TT", game: `${g.away} @ ${g.home}`, commence_time: g.commence_time, sport: g.sport, bookKey: book.key, bestOppName: g.bestOppName_under, isAlt: !!g.is_alt, ...resolveOpp({ trustedOpp: g.bestOpp_under, trustedBook: g.bestOpp_under_book, trustedCount: g.bestOppCount_under, trustedSize: g.bestOpp_under_size, sameBookOpp: overOdds, sameBookKey: book.key, sameBookSize: g.over_size, bookOdds: underOdds }) }, g.under_updatedAt)); }
      });
    }
  });

  // +EV is a cash bet. Underdog dk is the fee-inclusive phone price.
  return applyUnderdogCashLegPrices(legs.filter((l) => l.bestOpp != null), true);
}
...
function UnderdogStaleOddsBanner({ legs, now, style }) {
  const warn = describePromoUnderdogStaleWarning(legs, now);
  if (!warn) return null;
  return (
    <div
      role="status"
      title={warn.et ? `Betstamp last tick ${warn.et}` : undefined}
      style={{
        marginTop: 10,
        marginBottom: 4,
        padding: "8px 10px",
...
function UnderdogStaleOddsChip({ leg, now, style }) {
  const warn = describeUnderdogOfferStaleWarning(leg, now);
  if (!warn) return null;
  return (
    <div
      role="status"
      title={warn.et ? `Betstamp last tick ${warn.et}` : warn.message}
      style={{
        fontSize: 10,
        fontWeight: 700,
        lineHeight: 1.4,
                      ...
                      <div id={"pick-" + promoId} style={{ ...PROMO_CARD_LAYER_STYLE, background: i === 0 ? "rgba(59,130,246,0.06)" : "rgba(255,255,255,0.02)", border: `1px solid ${i === 0 ? "rgba(59,130,246,0.2)" : "rgba(255,255,255,0.06)"}`, borderRadius: 12, overflow: "hidden", cursor: "pointer" }}
                        ...
                        <div style={{ padding: "20px 24px" }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                            ...
                            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 22, fontWeight: 700, color: view.ev > 0 ? "#10b981" : "#ef4444" }}>{formatSignedEvMoney(view.ev)} EV</div>
                          </div>
                        ...
                        {isExpanded && (
                          <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "20px 24px", background: "rgba(0,0,0,0.2)" }}
                            ...
                            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 13, lineHeight: 1.8, color: "#9ca3af", padding: "14px 16px", background: "rgba(255,255,255,0.02)", borderRadius: 8, border: "1px solid rgba(255,255,255,0.06)", marginBottom: 12 }}>
                              ...
                              <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)", paddingTop: 6, marginTop: 6 }}>EV = <strong style={{ color: view.ev > 0 ? "#10b981" : "#ef4444" }}>{formatSignedEvMoney(view.ev)}</strong></div>
                            </div>
                            <div style={{ fontSize: 13, color: "#9ca3af", padding: "12px 16px", background: "rgba(16,185,129,0.04)", borderRadius: 8, border: "1px solid rgba(16,185,129,0.1)" }}>
                              <strong style={{ color: "#10b981" }}>Bottom line:</strong> This {isSingle ? "bet" : "parlay"} has a {(view.combinedProb * 100).toFixed(1)}% chance of hitting and pays <strong style={{ color: "#e8eaed" }}>${(p.boostedProfit + stake).toFixed(0)}</strong> with your boost. Expected profit: <strong style={{ color: view.ev > 0 ? "#10b981" : "#ef4444" }}>{formatSignedEvMoney(view.ev)}</strong> on a ${stake} bet.
                            </div>
                            {canSeeComboLocks(user) && (
                        ...
                        <div style={{ padding: "20px 24px" }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                            ...
                            <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end" }}>
                              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 22, fontWeight: 700, color: evColor }}>{formatSignedEvMoney(view.ev)} EV</div>
                              {p.isGuaranteed && (
                            ...
                            {p.isGuaranteed ? (
                              ...
                              <>
                                ...
                                <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 13, lineHeight: 1.8, color: "#9ca3af", padding: "14px 16px", background: "rgba(255,255,255,0.02)", borderRadius: 8, border: "1px solid rgba(255,255,255,0.06)", marginBottom: 12 }}>
                                  ...
                                  <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)", paddingTop: 6, marginTop: 6 }}>EV = <strong style={{ color: evColor }}>{formatSignedEvMoney(view.ev)}</strong></div>
                                ...
                                <div style={{ fontSize: 13, color: "#9ca3af", padding: "12px 16px", background: "rgba(16,185,129,0.04)", borderRadius: 8, border: "1px solid rgba(16,185,129,0.1)" }}>
                                  <strong style={{ color: "#10b981" }}>Bottom line:</strong> This {isSingle ? "bet" : "parlay"} has a {(view.combinedProb * 100).toFixed(1)}% chance of hitting and pays <strong style={{ color: "#e8eaed" }}>${(p.winProfit + stake).toFixed(0)}</strong>. If it loses, you get <strong style={{ color: "#e8eaed" }}>${p.refund.toFixed(0)}</strong> site credit (≈ <strong style={{ color: "#10b981" }}>${p.creditValue.toFixed(0)}</strong> cash). Expected profit: <strong style={{ color: evColor }}>{formatSignedEvMoney(view.ev)}</strong> on a ${stake} cash stake.
                    ...
                    return (
                      <div id={"pick-" + promoId} style={{ ...PROMO_CARD_LAYER_STYLE, background: i === 0 ? "rgba(139,92,246,0.06)" : "rgba(255,255,255,0.02)", border: `1px solid ${i === 0 ? "rgba(139,92,246,0.2)" : "rgba(255,255,255,0.06)"}`, borderRadius: 12, overflow: "hidden", cursor: "pointer" }}
                        ...
                        <div style={{ padding: "20px 24px" }}>
                          ...
                          <UnderdogStaleOddsBanner legs={overlay.displayLegs || p.legs} />
