// Overlay Betstamp BetCris (book id 642) mains onto Odds API Promo events.
// Odds API games stay the row identity. A Betstamp blip omits BetCris cells
// instead of failing the scan. Dropped Betstamp lines are stripped — never
// left as zombie prices (Odds API has no BetCris feed).
...
import { BETSTAMP_SPORTS, bookByKey } from "./betstampBooks.js";
import { betstampSnapshotUrl } from "./betstampLive.js";

export const BETCRIS_BOOK_KEY = "betcris";
export const BETCRIS_BOOK_ID = bookByKey(BETCRIS_BOOK_KEY)?.id ?? 642;
export const BETCRIS_COMMENCE_WINDOW_MS = 12 * 60 * 60 * 1000;
export const BETCRIS_FETCH_TIMEOUT_MS = 8000;
...
export function joinOddsEventToBetstampFixture(event, snapshot, { windowMs = BETCRIS_COMMENCE_WINDOW_MS } = {}) {
...
export function betcrisBookmakerFromSnapshot(event, snapshot, joinHit) {
  ...
  for (const market of asList(snapshot.markets, ["markets", "data"])) {
    ...
    if (bookKeyForMarket(market) !== BETCRIS_BOOK_KEY) continue;
  ...
  return { key: BETCRIS_BOOK_KEY, title: "BetCris", markets };
}

export function overlayBetcrisOnGame(game, snapshot) {
  if (!game || typeof game !== "object") return game;
  const bookmakers = (game.bookmakers || []).filter((b) => b && b.key !== BETCRIS_BOOK_KEY);
  const bm = snapshot ? betcrisBookmakerFromSnapshot(game, snapshot) : null;
  return { ...game, bookmakers: bm ? [...bookmakers, bm] : bookmakers };
}

export function overlayBetcrisOnGames(games, snapshot) {
  if (!Array.isArray(games)) return games;
  return games.map((game) => overlayBetcrisOnGame(game, snapshot));
}

export function overlayBetcrisOnCacheRows(rows, snapshot) {
  return (rows || []).map((row) => {
    if (!row) return row;
    if (Array.isArray(row.data)) {
      return { ...row, data: overlayBetcrisOnGames(row.data, snapshot) };
    }
    if (row.data && typeof row.data === "object") {
      return { ...row, data: overlayBetcrisOnGame(row.data, snapshot) };
    }
    return row;
  });
}

export function betcrisSnapshotUrl({ leagues } = {}) {
  const list = [...new Set((leagues || []).map((l) => String(l).toUpperCase()).filter((l) => l === "NFL" || l === "NCAAF"))];
  if (!list.length) return null;
  return betstampSnapshotUrl({
    league: list.join(","),
    live: false,
    bookIds: [BETCRIS_BOOK_ID],
  });
}

export async function fetchBetcrisSnapshot({
  leagues,
  fetchFn = fetch,
  timeoutMs = BETCRIS_FETCH_TIMEOUT_MS,
} = {}) {
  const url = betcrisSnapshotUrl({ leagues });
  if (!url) return null;
      ...
      ? await Promise.race([
          ...
          new Promise((_, reject) => {
            timer = setTimeout(() => {
              ...
              reject(new Error("betcris timeout"));
            }, timeoutMs);
          }),
        ])
      : await work;
