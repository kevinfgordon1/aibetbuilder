...

{
  ...
  assert.match(stamp, /data-best-view=\{effectiveBestView\}/);
  ...
  assert.match(stamp, /selectBoardMarket/);
  assert.match(stamp, /if \(next === "ml"\) setBestView\("single"\)/);
  assert.match(stamp, /const lineBestViews = market !== "ml"/);
  assert.match(stamp, /\{lineBestViews && \(/);
  assert.match(stamp, /On Spread and Totals, Best view default is Single/);
  assert.doesNotMatch(stamp, /moneyline stays single/);
